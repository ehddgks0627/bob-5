		...
		for (int i = 0; i < CHANNEL_SIZE; i++)
		{
			...
			if (data_cmp(&packet[IPV4_DIP_OFFSET], para.send_ip, IP_LEN) == 1 && data_cmp(&packet[ETH_SMAC_OFFSET], para.send_mac, MAC_LEN) == 1) //목적지ip가 나와 다른경우
			{
				...
				if (data_cmp(&packet[IPV4_SIP_OFFSET], relay[i].victim_ip, IP_LEN) == 0) //감염자가 보낸 패킷일경우
				{
					...
					unsigned char ipv4_proto = packet[IPV4_PROTO_OFFSET];
					if (ipv4_proto != TCP)
						goto SKIP;

					unsigned char ipv4_header_size = packet[IPV4_LENGTH_OFFSET];
					ipv4_header_size &= 0xF;
					ipv4_header_size *= 4;

					unsigned char port = packet[ETH_LENGTH + ipv4_header_size + TCP_DPORT_OFFSET1] * 0x100 + packet[ETH_LENGTH + ipv4_header_size + TCP_DPORT_OFFSET2];
					if (!(port == 80 || port == 8080))
						goto SKIP;

					unsigned char tcp_header_size = packet[ETH_LENGTH + ipv4_header_size + TCP_LENGTH_OFFSET];
					tcp_header_size &= 0xF0;
					tcp_header_size /= 0x10;
					tcp_header_size *= 4;

					unsigned char http = ETH_LENGTH + ipv4_header_size + tcp_header_size;
